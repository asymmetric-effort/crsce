...
int main() {
    ...
    // 1) No args → should succeed, no side effects
    {
        ...
        int rc = utils::parse_args(1, argv, opts);
    ...
    // 2) Required‐value happy path
    {
        ...
        int rc = utils::parse_args(3, argv, opts);
    ...
    // 3) Missing value for --foo → failure
    {
        ...
        int rc = utils::parse_args(2, argv, opts);
        tester.assertTrue(rc == EXIT_FAILURE, "Missing value → EXIT_FAILURE");
    }

    // 4) No‐value flag triggers early exit
    {
        fooVal.clear();
        barFlag = false;
        const char* argv[] = {"prog", "--bar", nullptr};
        int rc = utils::parse_args(2, argv, opts);
        tester.assertTrue(rc == EXIT_SUCCESS, "No‐value handler returns false → EXIT_SUCCESS");
    ...
    // 5) Short variants
    {
        ...
        int rc = utils::parse_args(4, argv, opts);
        tester.assertTrue(rc == EXIT_SUCCESS, "Short flags -f and -b");
        tester.assertTrue(fooVal == "abc",  "fooVal set by -f");
        tester.assertTrue(barFlag,          "barFlag set by -b");
    }
