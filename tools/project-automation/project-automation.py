...
def find_or_create_project(meta: dict) -> str:
    ...
    title = meta["title"]
    listing = run([
        "/usr/local/bin/gh", "project", "list",
        "--owner", owner,
        "--format", "json"
    ])
    projects = yaml.safe_load(listing)
    raw = run([
        "/usr/local/bin/gh", "project", "list", "--owner", owner, "--format", "json"
    ])
    try:
    ...
    # create new
    out = run([
        "/usr/local/bin/gh", "project", "create",
        "--owner", owner,
        "--title", title,
...
def ensure_fields(proj_id: str, fields: list[dict]) -> None:
    ...
    for fld in fields:
        ...
        subprocess.run(
            [
                "/usr/local/bin/gh", "project", "field-create", proj_id,
                "--name", name, "--type", typ
            ],
            check=False
        )
...
def sync_issues(proj_id: str, meta: dict, issues: list[dict]) -> None:
    ...
    owner = meta["owner"]
    repo = run(["/usr/local/bin/gh", "repo", "view", "--json", "name", "--jq", ".name"])
    for spec in issues:
        ...
        # add to project
        item_out = run([
            "/usr/local/bin/gh", "project", "item", "add", proj_id,
        ...
        for field, val in spec.get("fields", {}).items():
            run([
                "/usr/local/bin/gh", "project", "field-value", "set", item_id,
                "--field", field,
                "--value", val
            ])
