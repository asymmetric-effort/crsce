...
int CRSCE::compress() {
    try {
        ...
        // Load the entire input stream into inputBuffer
        ...
        while (readInputBuffer(inputBuffer)) {
            ...
            std::cerr << "[CRSCE] Read " << inputBuffer.size()
                    << " words (" << total_bits << " bits) "
                    << "into inputBuffer." << std::endl;
            ...
            while (buf_bit < total_bits) {
                std::cerr << "[CRSCE] Starting block "
                        << block_count << "/" << total_bits << std::endl;
                ...
                while (block_bits < s * s && buf_bit < total_bits) {
                    ...
                    const size_t bit_pos = (FILE_BUFFER_WIDTH - 1) - (buf_bit % FILE_BUFFER_WIDTH);
